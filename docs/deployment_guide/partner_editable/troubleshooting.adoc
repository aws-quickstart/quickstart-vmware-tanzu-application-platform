...
** EKS Cluster Name.
* The bastion hosts now uses {imds-v2}[Instance Metadata Service Version 2 (IMDSv2)]
* We changed how we check and deploy the shared resources stacks
  (`eks-quickstart-RegionalSharedResources` & `eks-quickstart-AccountSharedResources`) which results in a slightly faster deployments
